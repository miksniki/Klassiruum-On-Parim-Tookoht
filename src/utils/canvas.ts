...
export function context(canvas: HTMLCanvasElement) {
  ...
  if (!ctx)
    throw new Error(
      "Sinu brauser ei toeta pilditöötlust. Proovi uuemat brauserit."
    );
  return ctx;
}
  ...
  return new Promise((resolve, reject) =>
    canvas.toBlob(
      (blob) =>
        blob
          ? resolve(blob)
          : reject(new Error("Pildi salvestamine ebaõnnestus. Proovi uuesti.")),
      "image/png"
    )
  );
}
...
export function trimTransparentPixels(canvas: HTMLCanvasElement) {
  ...
  if (right < left)
    throw new Error(
      "Me ei leidnud pildilt inimest. Proovi selgemat portreefotot."
    );
