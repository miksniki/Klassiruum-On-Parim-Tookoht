...
export function enlargeEyes(
  ...
  const original = context(source).getImageData(
    0,
    0,
    source.width,
    source.height
    ...
    for (const eye of face.eyes) {
      ...
      for (let y = 0; y < height; y++)
        for (let x = 0; x < width; x++) {
          const dx = left + x - eye.x,
            dy = top + y - eye.y;
          const distance = Math.hypot(dx, dy) / radius;
          if (distance >= 1) continue;
          const scale =
            1 + (amount / 100) * 1.15 * (1 - distance * distance) ** 2;
          const sx = Math.max(
            0,
            Math.min(source.width - 1, eye.x + dx / scale)
          );
          const sy = Math.max(
            0,
            Math.min(source.height - 1, eye.y + dy / scale)
          );
          ...
          for (let c = 0; c < 4; c++) {
            const sample = (xx: number, yy: number) =>
              original.data[(yy * source.width + xx) * 4 + c];
...
export function drawGlasses(
  ...
  for (const {
    ...
    ctx.translate(0, settings.glassesY / 100);
    ...
    for (const x of [-0.5, 0.5]) {
      ctx.beginPath();
      if (settings.glasses === "round")
        ctx.ellipse(x, 0, 0.405, 0.39, 0, 0, Math.PI * 2);
      else
        ctx.roundRect(
          x - 0.415,
          -0.3,
          0.83,
          0.63,
          settings.glasses === "sun" ? 0.14 : 0.1
        );
      ctx.fillStyle =
        settings.glasses === "sun"
          ? "rgba(25, 35, 31, 0.9)"
          : "rgba(229, 243, 237, 0.08)";
      ctx.fill();
      ctx.stroke();
      ctx.save();
      ctx.clip();
      ...
      ctx.lineWidth = 0.025;
      ctx.beginPath();
      ctx.moveTo(x - 0.25, -0.18);
      ctx.lineTo(x - 0.08, -0.25);
      ctx.stroke();
      ctx.restore();
    }
    ctx.beginPath();
    ctx.moveTo(-0.09, -0.06);
    ctx.quadraticCurveTo(0, -0.15, 0.09, -0.06);
    ctx.moveTo(-0.91, -0.08);
    ctx.lineTo(-1.04, -0.16);
    ctx.moveTo(0.91, -0.08);
    ctx.lineTo(1.04, -0.16);
    ctx.stroke();
    ctx.restore();
